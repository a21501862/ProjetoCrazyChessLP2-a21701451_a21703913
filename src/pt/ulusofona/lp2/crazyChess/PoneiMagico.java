            ...
            if(count == pecasJogo.size()){
                ...
                for(CrazyPiece peca1 : pecasJogo){
                    ...
                    if (count == pecasJogo.size() && valida){
                        count = 0;
                        for(CrazyPiece existePeca : pecasJogo){
                            if (existePeca.getIdEquipa() != idEquipa && existePeca.getX() == xO + 2 && existePeca.getY() == yO - 2){
                                jogadaPonei.add(new Sugestao(xO + 2, yO - 2, existePeca.getPontos()));
                                break;
                            }
                            count++;
                            if (count == pecasJogo.size()){
                                jogadaPonei.add(new Sugestao(xO + 2, yO - 2, 0));
                            }
                        }
                    }
                }
